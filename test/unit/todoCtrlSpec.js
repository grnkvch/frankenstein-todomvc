	...
	describe('Todo Controller', function () {
		...
		describe('having no Todos', function () {
			...
			it('should trim whitespace from new Todos', function () {
				...
				expect(scope.todos[0].text).toBe('buy some unicorns');
			});
		});
		...
		describe('having some saved Todos', function () {
			...
			beforeEach(inject(function ($controller) {
				...
				store.insert({ text: 'Uncompleted Item 0', completed: false });
				store.insert({ text: 'Uncompleted Item 1', completed: false });
				store.insert({ text: 'Uncompleted Item 2', completed: false });
				store.insert({ text: 'Completed Item 0', completed: true });
				store.insert({ text: 'Completed Item 1', completed: true });
				scope.$digest();
			}));
			...
			it('should trim Todos on saving', function () {
				...
				scope.editTodo(todo);
				todo.text = ' buy moar unicorns  ';
				scope.saveEdits(todo);
				expect(scope.todos[0].text).toBe('buy moar unicorns');
			});
			...
			it('revertTodo() get a Todo to its previous state', function () {
				var todo = store.todos[0];
				scope.editTodo(todo);
				todo.text = 'Unicorn sparkly skypuffles.';
				scope.revertEdits(todo);
				scope.$digest();
				expect(scope.todos[0].text).toBe('Uncompleted Item 0');
			});
		});
	});
}());
